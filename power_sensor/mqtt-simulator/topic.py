...
class Topic(ABC):
    def __init__(self, broker_url: str, broker_port: int, broker_protocol: int, topic_url: str,
                 topic_data: list[object], topic_client_settings: ClientSettings):
...
class TopicAuto(Topic, threading.Thread):
    ...
    def generate_initial_value(self, data):
        if 'INITIAL_VALUE' in data:
            ...
            self.expression_evaluators[data['NAME']] = ExpressionEvaluator(data['MATH_EXPRESSION'],
    ...
    def generate_next_value(self, data, old_value):
        ...
        if data.get('RESTART_ON_BOUNDARIES', False) and (
                old_value == data.get('MIN_VALUE') or old_value == data.get('MAX_VALUE')):
            return self.generate_initial_value(data)
        ...
        if data['TYPE'] == 'bool':
            ...
            return round(self.expression_evaluators[data['NAME']].evaluate_expression(), 2)
        elif data['TYPE'] == 'raw_values':
            ...
            step = round(step) if data['TYPE'] == 'int' else round(step, 2)
